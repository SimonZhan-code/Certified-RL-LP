...
from examples.AttControl.handelman_utils import possible_handelman_generation
from examples.Ball.Ball4_LP import monomial_generation
...
def LyapunovConsGenerate():
						...
						if i + j + k + g <= degree:
							if exp1.coeff(y, i).coeff(v_y, j).coeff(phi_e, k).coeff(r, g) != 0:
								if exp2.coeff(y, i).coeff(v_y, j).coeff(phi_e, k).coeff(r, g) != 0:
									file.write('constraints += [', exp1.coeff(y, i).coeff(v_y, j).coeff(phi_e, k).coeff(r, g), ' <= ', exp2.coeff(y, i).coeff(v_y, j).coeff(phi_e, k).coeff(r, g), '+ objc',']')
									file.write('constraints += [', exp1.coeff(y, i).coeff(v_y, j).coeff(phi_e, k).coeff(r, g), ' >= ', exp2.coeff(y, i).coeff(v_y, j).coeff(phi_e, k).coeff(r, g), '- objc',']')
								else:
									file.write('constraints += [', exp1.coeff(y, i).coeff(v_y, j).coeff(phi_e, k).coeff(r, g), ' == ', exp2.coeff(y, i).coeff(v_y, j).coeff(phi_e, k).coeff(r, g), ']')
	...
	generateConstraints(rhs_der, lhs_der, degree=3)	
