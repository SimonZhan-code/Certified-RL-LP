...
def LyapunovConstraints():
	...
	Xbase = [a,b,c,d,e,f]
	power6 = monomial_generation(6, Xbase)
	power6Base = Matrix(power6)
	ele = Matrix(monomial_generation(3, Xbase))

	NewBase = Matrix(monomial_generation(4, Xbase))


	V = MatrixSymbol('V', 1, len(power6))
	...
	print(len(ele))
	print(len(NewBase))
	file = open("cons_SDP_deg6.txt","w")
	Lya = V*power6Base - m*Matrix([2 - a**2 - b**2 - c**2 - d**2 - e**2 - f**2])
	Lya = expand(Lya[0, 0])
	rhsX = ele.T*X*ele
	rhsX = expand(rhsX[0, 0])
	generateConstraints(a,b,c,d,e,f, rhsX, Lya, file, degree=6)
	...
	generateConstraints(a,b,c,d,e,f, rhsY, lhsY, file, degree=8)
