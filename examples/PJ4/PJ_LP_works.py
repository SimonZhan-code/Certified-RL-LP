...
def senGradLP(control_param, l, f, g):
	...
	constraints += [0.5*lambda_1[0, 0] - 0.5*lambda_1[0, 1] + 2*lambda_1[0, 2] - lambda_1[0, 3] + 0.25*lambda_1[0, 4] + 0.25*lambda_1[0, 5] + 4*lambda_1[0, 6] + lambda_1[0, 7] - 0.25*lambda_1[0, 8] + 1.0*lambda_1[0, 9] - 1.0*lambda_1[0, 10] - 0.5*lambda_1[0, 11] + 0.5*lambda_1[0, 12] - 2*lambda_1[0, 13] >= B[0, 0] - objc - 0.5]
	constraints += [0.5*lambda_1[0, 0] - 0.5*lambda_1[0, 1] + 2*lambda_1[0, 2] - lambda_1[0, 3] + 0.25*lambda_1[0, 4] + 0.25*lambda_1[0, 5] + 4*lambda_1[0, 6] + lambda_1[0, 7] - 0.25*lambda_1[0, 8] + 1.0*lambda_1[0, 9] - 1.0*lambda_1[0, 10] - 0.5*lambda_1[0, 11] + 0.5*lambda_1[0, 12] - 2*lambda_1[0, 13] <= B[0, 0] + objc - 0.5]
...
def unsafeValidTest(Barrier_param):
	...
	for i in range(10000):
		...
		# 	x1 = np.random.uniform(low=-0.3, high=-1.3, size=1)[0]
		# 	x2 = np.random.uniform(low=-0.5, high=-1.5, size=1)[0]
		x1 = np.random.uniform(low=-1, high=-0.6, size=1)[0]
		x2 = np.random.uniform(low=-1.2, high=-0.8, size=1)[0]
		while (x1 + 0.8)**2 + (x2 + 1)**2 - 0.04 > 0:
			x1 = np.random.uniform(low=-1, high=-0.6, size=1)[0]
			x2 = np.random.uniform(low=-1.2, high=-0.8, size=1)[0]
...
def plot(control_param, Barrier_param, figname, N=5, Barrier=True):
	...
	circle2 = plt.Circle((-0.8, -1), 0.2, color='r')
